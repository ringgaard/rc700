...
// The function disasm() is the only global function of this module. The first 
// argument is a pointer to a unsigned char pointer, which points to the 
// op-code to disassemble. The output of the disassembly goes to stdout, 
// terminated by a newline. After the disassembly the pointer to the op-code 
// will be increased by the size of the op-code, so that disasm() can be called
// again. The second argument is the (Z80) address of the op-code to 
// disassemble. It is used to calculate the destination address of relative 
// jumps.
